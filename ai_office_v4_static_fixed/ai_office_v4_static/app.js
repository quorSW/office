...
function sendToDesk(agent) {
  ...
  const ty = agent.desk.y + agent.desk.h + 20;
  agent.status = 'transit';
  agent.seated = false;
...
function drawFloor() {
  ctx.fillStyle = '#d7bf98';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  for (let x = 0; x < canvas.width; x += 28) {
    for (let y = 0; y < canvas.height; y += 28) {
      ctx.fillStyle = ((x + y) / 28) % 2 === 0 ? '#dcc6a1' : '#d2bb94';
...
function drawRooms() {
  ...
  room(770, 90, 170, 190, 'Кофейный уголок');
...
function room(x, y, w, h, label) {
  ...
  ctx.strokeStyle = 'rgba(112,79,45,.18)';
  ctx.lineWidth = 2;
...
function drawCorridors() {
  ctx.fillStyle = '#caa978';
  ctx.fillRect(80, 255, 860, 40);
  ctx.fillRect(80, 455, 860, 40);
  ctx.fillStyle = 'rgba(255,255,255,.18)';
  for (let x = 100; x < 900; x += 44) {
    ctx.fillRect(x, 273, 20, 4);
    ctx.fillRect(x, 473, 20, 4);
  }
}

function drawDesk(d) {
  ctx.fillStyle = '#8d6945';
  roundRect(d.x, d.y, d.w, d.h, 12, true, false);
  ctx.fillStyle = '#6c4f34';
  roundRect(d.x + 10, d.y + 8, d.w - 20, 12, 6, true, false);
  ctx.fillStyle = '#b9d7d6';
...
function drawAgent(agent) {
  // status halo
  const statusColor = agent.status === 'working' ? '#50835c' : agent.status === 'break' ? '#a46b58' : agent.status === 'transit' ? '#d88b4c' : '#8a725a';
  ...
  ctx.fillStyle = '#8a725a';
  if (!agent.seated || agent.status !== 'idle') {
...
log('Офис готов к запуску. Сегодня в офисе спокойно и уютно.');
syncUI();
render();
